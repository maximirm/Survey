...
def get_question(db: Session, question_id: UUID):
    ...
    if db_question is None:
        raise QuestionNotFoundException(f"Question with ID {str(question_id)} not found")
    return db_question
...
def get_questions_by_survey_id(db: Session, survey_id: UUID):
    ...
    if not db_questions:
        raise QuestionNotFoundException(f"No Questions for Survey-ID {str(survey_id)} found")
    return db_questions
...
def create_question(db: Session, question: schemas.QuestionCreate):
    try:
        ...
        raise ForeignKeyNotFoundException(f"Can not create Question. No Survey with ID {str(question.survey_id)} found")
